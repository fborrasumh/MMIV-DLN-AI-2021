...
# CAM and grad-CAM for the tumor grade example, from fastai_amalgam
...
def img2arraylike(img:Image.Image, as_array=False, as_tensor=True):
    img = np.array(img).transpose(2,0,1)
    if as_array: return img
    if as_tensor: return torch.from_numpy(img)

def make_img_grid(images : Union[List[str], List[PILImage], List[Image.Image]],
    ...
    if isinstance(images[0], (PILImage, Image.Image)):
        img_list = images
